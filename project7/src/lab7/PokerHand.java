...
public class PokerHand
{
  ...
  public boolean isStraight()
  {
    ...
    for(int i = 1;i < cards.length;i++){
    		previousCardRank = cards[i - 1].getRank();
    		currentCardRank = cards[i].getRank();
  ...
  private int findIndexOfMinRank(int startingPoint){
	  int indexOfMin = startingPoint;
	  
	  //check cards to the right
	  for(int w = startingPoint + 1;w < cards.length;w++){
		  Card min = cards[indexOfMin];
		  Card current = cards[w];
  ...
  private void sortNoSuit()
  {
	  ...
	  //for loop that goes through hand
	  for(int i = 0;i < cards.length - 1;i++){
		  int indexOfMin = findIndexOfMinRank(i);
		  swap(i, indexOfMin);
	  }
